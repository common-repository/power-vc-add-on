...
if ( !class_exists( 'Wn_Image_Class' ) ) {
	require_once POWER_ADDONS_DIR .'includes/class-webnus-image.php';
}
...
class Wn_Img_Maniuplate {
	...
	public function m_image( $img , $width , $height ) {

		try {

			$name = basename( $img );

			$image = new \Wn_Image_Class( $img ); // Create a new webnus image object

			$image->fromFile( $img );// load image

			$image->thumbnail( $width, $height);// resize

			$out = $image->toFile( '', null , 100 , $name , $width , $height ); //to file

			return $out; // return url 
